...
class CourseModuleMarks extends Page
{
    ...
    private function branchEnrolledUserIds()
    {
        $branchId = auth()->user()->branch_id;

        return Enrollment::where('course_id', $this->record->id)
    ...
    public function getModuleTabsData(): array
    {
        ...
        $enrolledUserIds = $this->branchEnrolledUserIds();
    ...
    public function exportExcel(): \Symfony\Component\HttpFoundation\StreamedResponse
    {
        ...
        array_unshift($rows, ["Branch: {$branchName}", '', '', '', '', '', '']);
    ...
    {
        // Only allow deleting attempts for students in this branch
        $branchId = auth()->user()->branch_id;
        $student  = \App\Models\User::where('id', $userId)
        ...
        if ($deleted) {
            ...
            Notification::make()->title('No attempts found')->warning()->send();
        }
    }
}
