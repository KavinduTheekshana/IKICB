...
use App\Filament\Branch\Resources\AttendanceResource\Pages;
...
class AttendanceResource extends Resource
{
    ...
    // Scope to meetings that belong to courses with branch students
    public static function getEloquentQuery(): Builder
    {
        $branchId = auth()->user()->branch_id;

        return ModuleMeeting::query()
            ->with(['module.course', 'attendances'])
            ->whereHas('module.course', function ($q) use ($branchId) {
                $q->whereHas('enrollments.user', function ($q2) use ($branchId) {
        ...
        return $table
            ...
            ->actions([
                Tables\Actions\Action::make('manage_attendance')
                    ...
                    ->color('warning')
                    ->form(function ($record) use ($branchId) {
                        $moduleId = $record->module_id;

                        // Only branch students enrolled in this module's course
                        $students = User::where('role', 'student')
                            ->where('branch_id', $branchId)
                            ->where(function ($q) use ($moduleId, $record) {
                                $q->whereHas('enrollments', fn ($q2) => $q2
                                    ->where('course_id', $record->module->course_id)
                                    ->where('status', 'active'))
