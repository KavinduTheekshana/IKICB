                ...
                <div class="relative">
                    <div class="aspect-square rounded-3xl bg-gradient-to-br from-yellow-100 to-yellow-200 p-8">
                        <div class="w-full h-full rounded-2xl bg-white shadow-2xl overflow-hidden">
                            <!-- Replace 'about-image.jpg' with your actual image path -->
                            <img src="{{ asset('images/make-up-artist-applying-eyeshadow-face-with-brush-2.jpg') }}" alt="About IKICB" class="w-full h-full object-cover">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                @forelse($instructors as $instructor)
                    <div class="bg-white rounded-3xl shadow-lg overflow-hidden border border-gray-100 hover:shadow-2xl transition-all duration-300 card-hover">
                        ...
                        <div class="aspect-square bg-gradient-to-br from-yellow-100 to-yellow-200 flex items-center justify-center relative overflow-hidden">
                            @if($instructor->image)
                                <img src="{{ asset('storage/' . $instructor->image) }}" alt="{{ $instructor->name }}" class="w-full h-full object-cover">
                            @else
                        ...
                        <div class="p-6">
                            <h3 class="text-2xl font-black text-gray-900 mb-2">{{ $instructor->name }}</h3>
                            <p class="text-yellow-600 font-bold mb-4">{{ $instructor->position }}</p>
                            <div class="space-y-2">
                                @foreach($instructor->qualifications as $qualification)
                                    <div class="flex items-start space-x-2">
                                        <svg class="w-5 h-5 text-yellow-600 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                        </svg>
                                        <span class="text-gray-700 text-sm">{{ $qualification }}</span>
                                    </div>
                                @endforeach
                            </div>
                        </div>
                ...
                @empty
                    <div class="col-span-full text-center py-12">
                        <p class="text-gray-500 text-lg">No instructors available at the moment.</p>
                    </div>
                @endforelse
            </div>
        </div>
